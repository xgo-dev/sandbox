...
// Modified for sandbox: private object types and byte-slice output.
//
...
type encodeState struct {
	...
	encodedStructs map[reflect.Value]*structValue
...
func (es *encodeState) encodeMap(obj reflect.Value, dest *object) {
	if obj.IsNil() {
...
func (es *encodeState) encodeStruct(obj reflect.Value, dest *object) {
...
func (es *encodeState) encodeObject(obj reflect.Value, how encodeStrategy, dest *object) {
	...
	case reflect.Ptr: // Fast path: first.
		...
		*dest = r
		...
		es.resolve(obj, r)
	case reflect.Bool:
		*dest = boolValue(obj.Bool())
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		*dest = intValue(obj.Int())
	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64, reflect.Uintptr:
		*dest = uintValue(obj.Uint())
	case reflect.Float32:
		*dest = float32Value(obj.Float())
	case reflect.Float64:
		*dest = float64Value(obj.Float())
	case reflect.Complex64:
		c := complex64Value(obj.Complex())
		*dest = &c // Needs alloc.
	case reflect.Complex128:
		c := complex128Value(obj.Complex())
		*dest = &c // Needs alloc.
	case reflect.String:
		s := stringValue(obj.String())
		*dest = &s // Needs alloc.
	case reflect.Array:
		es.encodeArray(obj, dest)
	case reflect.Slice:
		s := &sliceValue{
			Capacity: uintValue(obj.Cap()),
			Length:   uintValue(obj.Len()),
		}
		*dest = s
		// Note that we do need to provide a sliceValue type here as
		// how is not encodeDefault. If this were the case, then it
		// would have been caught by the IsZero check above and we
		// would have just used nilValue{}.
		if obj.IsNil() {
			return
		}
...
func (es *encodeState) Save(obj reflect.Value) {
	...
	if err := safely(func() {
		...
		for _, id := range ids {
			// Encode the id.
			oes = nil
			if err := es.w.put(uintValue(id)); err != nil {
				panic(err)
			}
			// Marshal the object.
			oes = es.pending[id]
			if err := es.w.put(oes.encoded); err != nil {
				panic(err)
			}
		}
	}); err != nil {
...
func writeHeader(w *writer, length uint64, object bool) error {
